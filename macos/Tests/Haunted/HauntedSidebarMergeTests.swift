...
/// The sidebar's console-nodes × local-Lima-VMs join (MERGE-*).
...
struct HauntedSidebarMergeTests {
    ...
    @Test("MERGE-01: a console node and its local VM become one row (prefixed names)")
    func mergesByName() {
        // The console stores "alice-box"; the local VM and the display use
        // the bare "box".
        let rows = HauntedSidebarMerge.mergeRows(
            nodes: [Self.node("alice/alice-box/haunted")],
            lima: [Self.vm("box")],
            ops: [:],
            username: "alice")
        #expect(rows.count == 1)
        #expect(rows[0].node?.target == "alice/alice-box/haunted")
    ...
    func legacyUnprefixedMerges() {
        let rows = HauntedSidebarMerge.mergeRows(
            nodes: [Self.node("alice/box/haunted")],
            lima: [Self.vm("box")],
            ops: [:],
            username: "alice")
        #expect(rows.count == 1)
    ...
    func unmatchedRowsStandAlone() {
        let rows = HauntedSidebarMerge.mergeRows(
            nodes: [Self.node("alice/remote/haunted")],
            lima: [Self.vm("fresh", status: "Stopped")],
            ops: [:],
            username: "alice")
        #expect(rows.count == 2)
        let console = rows.first { $0.node != nil }
        let local = rows.first { $0.node == nil }
        #expect(console?.lima == nil, "a remote node gets no Lima menu")
    ...
    func nilUsernameMergesNothing() {
        let rows = HauntedSidebarMerge.mergeRows(
            ...
            username: nil)
        #expect(rows.count == 2)
        #expect(rows.allSatisfy { $0.lima == nil || $0.node == nil })
    }
    ...
    func orphanCarriesOp() {
        let rows = HauntedSidebarMerge.mergeRows(
            nodes: [Self.node("alice/alice-gone/haunted", online: false)],
            lima: [],
            ops: ["gone": .deleting],
            username: "alice")
        #expect(rows.count == 1)
    ...
    func ordering() {
        ...
        let rows = HauntedSidebarMerge.mergeRows(
            nodes: [Self.node("alice/alice-zeta/haunted")],
    ...
    func vmClaimsOwnedRowOnly() {
        let rows = HauntedSidebarMerge.mergeRows(
            nodes: [
                Self.node("bob/box/haunted"),
                Self.node("alice/alice-box/haunted"),
            ],
            lima: [Self.vm("box")],
            ops: [:],
            username: "alice")
        #expect(rows.count == 2)
        let mine = rows.first { $0.node?.target == "alice/alice-box/haunted" }
        let theirs = rows.first { $0.node?.target == "bob/box/haunted" }
